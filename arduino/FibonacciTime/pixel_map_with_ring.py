...
data = np.column_stack([data[:,0] - np.mean(data[:,0]),
                        data[:,1] - np.mean(data[:,1])])
                        
                                            
dist = np.amax(np.linalg.norm(data[np.newaxis] - data[:,np.newaxis], axis=2))
r_pre = max(np.linalg.norm(data, axis=1))
r_post = 168/2
scale = r_post / r_pre
data = scale * data
...
d = c / np.pi
r = d / 2
theta = 2 * np.pi * np.arange(n_ring) / n_ring + 5 * DEG
ring_x = -r * np.cos(theta)
ring_y =  r * np.sin(theta)
pl.plot(data[:,0], data[:,1], 'go')
pl.plot(ring_x, ring_y)
pl.plot(ring_x, ring_y, 'gs')
pl.plot(ring_x[0], ring_y[0], 'rs')
pl.axis('equal')
pl.plot(data[-1, 0], data[-1, 1], 'ro')

print('''\
#ifndef LAYOUT_H
#define LAYOUT_H
''')
print('int layout_x[%d] = {' % (256 + 84))
for i in range(16):
    print('    ', end='')
    for j in range(16):
        print('%3d' % int(data[i * 16 + j, 0]), end=',')
    print()
...
print('int layout_y[%d] = {' % (256 + 84))
for i in range(16):
    print('    ', end='')
    for j in range(16):
        print('%3d' % int(data[i * 16 + j, 1]), end=',')
    print()
